...
BondBoost::BondBoost(Matter *matt,Parameters *params): Hyperdynamics(matt, params) {
    matter = matt;    
    parameters = params;
...
*/
    ...
    for (i = 0; i < nBBs; i++) {
        ...
        //        i, Ri[0], Ri[1], Ri[2], sqrt(Ri[0] * Ri[0] + Ri[1] * Ri[1] +
        //        Ri[2] * Ri[2]),R);
...
/*
    ...
    printf("OldForces::\n");
        ...
        for(i = 0; i < nAtoms; i++){
            printf("%lf   %lf   %lf\n", OldForce(i, 0), OldForce(i, 1),
                   OldForce(i, 2));
        }
*/

    // printf("boost_fact= %lf, totE= %lf\n",
    //        Boost_Fact, Boost_Fact + matter->getKineticEnergy() +
