...
This forcefield is the A2 water-platinum potential invented by @ref zhu1994 "Zhu and Philpott". It includes the SPC/E+CCL potential for interaction between the molecules of water. The SPC/E is a potential for constrained water. This implementation includes restraints so it may be used without constraint (see class SpceCcl).\n
The potential uses Kong's rules to combine the Lennard-Jones parameters of platinum with oxygen and hydrogen. A @ref combinationReview "review of combination rules" including Kong's rule can be find on the web.\n
...
Unlike Lennard-Jones Parameters for Vapor-Liquid Equilibria, Thorsten Schnabel, Jadran Vrabec , Hans Hasse,
Institut fur Technische Thermodynamik und Thermische Verfahrenstechnik,
Universitat Stuttgart, D-70550 Stuttgart, Germany, http://www.itt.uni-stuttgart.de/~schnabel/CR.pdf.
@internal The class contains many functions with some of them calculating only one type of interaction. Those functions often have a parameter called @a force and one @a energy . Some of those functions simply return the force and energy into the variables provided while others add the force and energy to the values already stored in the variables. Look at the documentation of the function to know how it operates. The parameter is marked as both input and ouput then it adds the values, otherwise it is simply marked as output.
*/
...
template <class P>
ZhuPhilpott<P>::ZhuPhilpott(ZhuPhilpott const & zhuPhilpott)
{
...
The order of the atoms is all hydrogens (hydrogens belonging to the same molecule are next to each others), all oxygen, all platinum.
The length of arrays @a r and @a f is @f$ 3\times(3 \times nWater+nPt) @f$.
The length of array @a fixed is @f$ 3 \times nWater+nPt @f$.
When @a fixed is provided the interaction between two fixed atoms may be skipped to speed up the calculation. The parameter is optional.
@warning  The conductivity is simulated by interaction a mirror images of charges. The mirror surface is set at z=0, which should correspond to the location of the highest layer of platinum atoms.
*/
template <class P>
void ZhuPhilpott<P>::computeHH_O_Pt_(
                                     const int nWater, const int nPt,
                                     const double r[], double f[], double & energy,
                                     double const b[], bool const fixed[])
{
    const double (*const rh1)[6]=reinterpret_cast<const double(*)[6]>(r);
    const double (*const rh2)[6]=reinterpret_cast<const double(*)[6]>(&r[3]);
    const double (*const ro)[3]=reinterpret_cast<const double(*)[3]>(&r[nWater*6]);
    const double (*const rPt)[3]=reinterpret_cast<const double(*)[3]>(&r[nWater*9]);
    ...
    if (fixed) {
        ...
        bool const (*const xo)[1]=reinterpret_cast<bool const (*)[1]>(&fixed[nWater*2]);
        bool const *const xPt=&fixed[nWater*3];
        computeTemplate(nWater, rh1, rh2, ro, fh1, fh2, fo, nPt, rPt, fPt, energy, b, xh1, xh2, xo, xPt);
    }
    else {
        bool const (*const xh1)[2]=0;
        bool const (*const xh2)[2]=0;
        bool const (*const xo)[1]=0;
        computeTemplate(nWater, rh1, rh2, ro, fh1, fh2, fo, nPt, rPt, fPt, energy, b, xh1, xh2, xo, 0);            
    };
}
...
The positions of the platinum atoms must have been previously provided through function setPlatinum().
@param[in] nWater Number of molecules of water.
...
@note The function does not return the forces on platinum atoms as they as assumed to be fixed.
@see setPlatinum() and computeHH_O_Pt_().
*/
template <class P>
void ZhuPhilpott<P>::computeHH_O_(
                                  const int nWater,
                                  const double r[], double f[], double & energy,
                                  double const b[], bool const fixed[])
{
    ...
    const double (*const rh2)[6]=reinterpret_cast<const double(*)[6]>(&r[3]);
    const double (*const ro)[3]=reinterpret_cast<const double(*)[3]>(&r[nWater*6]);
    const double (*const rPt)[3]=reinterpret_cast<const double(*)[3]>(positions_);
    ...
    if (fixed) {
        ...
        bool const (*const xh2)[2]=reinterpret_cast<bool const (*)[2]>(&fixed[1]);
        bool const (*const xo)[1]=reinterpret_cast<bool const (*)[1]>(&fixed[nWater*2]);
        computeTemplate(nWater, rh1, rh2, ro, fh1, fh2, fo, nPlatinum_, rPt, fPt, energy, b, xh1, xh2, xo, 0);
    }
    else {
        bool const (*const xh1)[2]=0;
        bool const (*const xh2)[2]=0;
...
/// @see setPlatinum().
template <class P>
int ZhuPhilpott<P>::nPlatinum() const
{
    return nPlatinum_;
}
...
template <class P>
char const * ZhuPhilpott<P>::getName()
{
    return "ZhuPhilpott";
}
...
void ZhuPhilpott<P>::computeTemplate(
                                     ...
                                     double & energy,
                                     double const b[],
                                     bool const (*const xh1)[H],
                                     bool const (*const xh2)[H],
                                     ...
                                     bool const xPt[])
{
    ...
    for (int i=nWater-1; i >= 0; --i) {
        ...
        interactWithCorePt(w1, nPt, rPt, fPt, energy);// called Vw-core in Zhu and Philpott
        assert(not isnan(energy));
...
*/
template <class P>
void ZhuPhilpott<P>::interactWithCorePt(Water & water, int const nPt, double const rPt[][3], double fPt[][3], double & energy)
{
    for (int i=0; i < nPt; ++i) {
...
template <class P>
void ZhuPhilpott<P>::interactionPtO(double const R1[], double const R2[], double F1[], double F2[], double & energy)
{
...
template <class P>
void ZhuPhilpott<P>::interactionPtH(double const R1[], double const R2[], double F1[], double F2[], double & energy)
{
    double R12[3], F12[3]={0}, r1, f1=0; 
    distance(R1, R2, R12, r1);
...
@param[in, out]      force         Vector force created by the interaction between the two atoms.
...
template <class P>
void ZhuPhilpott<P>::anisotropic(const double distance[], double force[], double & energy, double const epsilon, double const sigma, double const alpha)
{
    ...
    f=2.0*(dE_da*pow(a, 2.0)*alpha2+dE_db*pow(b, 2.0)/alpha2)/pow(sigma, 2.0);
...
@param[in,out]     force   Force create by the interaction between the two atoms.
@param[in,out]      energy    Energy created by the interaction.
@param[in] epsilon See equation.
@param[in] sigma See equation.
...
@internal The subroutine has not been optimised.*/
template <class P>
void ZhuPhilpott<P>::isotropic10(double const distance, double & force, double & energy, double const epsilon, double const sigma, double const C10)
{
...
@note Does not include the interaction of @a w2 with the image of @a w1. For this you must explicitely call interactWithImage(w2, w1, U).
*/
template <class P>
void ZhuPhilpott<P>::interactWithImage(Water & w1, Water & w2, double & U)
{
    ...
    // -2 is given as the relative permittivity. This value is not the permittivity of the metal which is infinite in the case of a perfect metal. It is the value (1+epsilon_r)/(1-epsilon_r) which is equal to -1 for a perfect metal. This value is the 'effective' permittivity for the interaction of a charge with a mirror image.
...
The precondition for using the function is that the distance between @a w1 and @a w2 defined as @f$ |\mathbf r_{n1}-\mathbf r_{n2}| @f$ verifies the equation:
@f[
...
*/
template <class P>
void ZhuPhilpott<P>::coulombWithCutoff(Water & w1, Water & w2, double & U, double const relativePermittivity)
{
    double z[3], z1, z2;
    ...
    if (z1 <= cutoff_ - switchingWidth_) {
        ...
        switching(
                  g1, g2,
                  energy, cutoff_, switchingWidth_);
        U += energy;
        w1.addForces(v1);
        w2.addForces(v2);
    };
...
*/
template <class P>
void ZhuPhilpott<P>::coulombFull(Water & w1, Water & w2, double & U, double const relativePermittivity)
{
