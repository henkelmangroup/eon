...
std::vector<std::string> SafeHyperJob::run(void)
{
    ...
    final_state = new Matter(parameters);
...
int SafeHyperJob::dynamics()
{
    ...
    StateCheckInterval =
        int(parameters->parrepStateCheckInterval / parameters->mdTimeStep);
    RecordInterval =
        int(parameters->parrepRecordInterval / parameters->mdTimeStep);
    ...
    while(!stopFlag) {
        if ((parameters->biasPotential == Hyperdynamics::BOND_BOOST)
            && !newStateFlag ) {
            // GH: boost should be a unitless factor, multipled by TimeStep to get
            // the boosted time
        ...
        nCheck++; // count up to parameters->parrepStateCheckInterval before
        ...
        if (transitionFlag) {
            ...
            *current = *mdBuffer[refineStep-1];
            ...
            log("tranisitonTime= %.3e s, biasPot= %.3f eV, "
                "correctedTime= %.3e s, sumCorrectedTime= %.3e s, "
                "minCorTime= %.3e s\n",
                transitionTime * 1e-15 * parameters->timeUnit,
                transitionPot, correctedTime * 1e-15 * parameters->timeUnit,
                sumCorrectedTime * 1e-15 * parameters->timeUnit,
                minCorrectedTime * 1.0e-15 * parameters->timeUnit);
        ...
        if ((step % tenthSteps == 0) || (step == parameters->mdSteps)) {
            ...
            log("progress: %3.0f%%, max displacement: %6.3lf, step %7ld/%ld\n",
                (double)100.0 * step / parameters->mdSteps,
                maxAtomDistance, step, parameters->mdSteps);
        }
    }
    ...
    if (nBoost > 0){
        log("\nTemperature : Average = %lf ; Stddev = %lf ; "
            "Factor = %lf; Boost = %lf\n\n",
...
void SafeHyperJob::dephase()
{
    ...
    while (step < DephaseSteps) {
        ...
        if (transitionFlag) {
            ...
            log("Dephasing warning: in a new state, inverse the "
                "momentum and restart from step %ld\n",
                step + transitionStep);
        ...
        if ((parameters->parrepDephaseLoopStop) &&
            (loop > parameters->parrepDephaseLoopMax)) {
