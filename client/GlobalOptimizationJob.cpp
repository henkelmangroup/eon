...
std::vector<std::string> GlobalOptimizationJob::run(void)
{
    ...
    for(long istep = 1; istep <= nstep; istep++) {
        ...
        report(matter_hop);
        if(matter_cur->getPotentialEnergy() < parameters->globalOptimizationTargetEnergy)
            break;
    }
...
void GlobalOptimizationJob::examineEscape(Matter *matter_cur,Matter *matter_hop) {
    ...
    //    matter_cur->getPotentialEnergy(),
    //    matter_hop->getPotentialEnergy() - matter_cur->getPotentialEnergy());
...
void GlobalOptimizationJob::mdescape(Matter *matter)
{
    ...
    if (md_presumably_escaped) {
        devcon = devcon / (double)(3*matter->numberOfFreeAtoms());
...
void GlobalOptimizationJob::velopt(Matter *matter)
{
    ...
    long nFreeCoords = 3*matter->numberOfFreeAtoms();
...
{
    // epot is in interval [earr(jlo),earr(jlo+1)] ;
    // earr(0)=-Infinity ; earr(n+1) = Infinity
    size_t jlo;
    double de;
    ...
    // if (jlo != earr.size() - 1)
    //     if (abs(epot - earr[jlo + 1]) < parameters->energyDifference)
    //         jlo++;
    return jlo;
}
