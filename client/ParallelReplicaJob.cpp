...
ParallelReplicaJob::ParallelReplicaJob(Parameters *params) { parameters = params; }
...
std::vector<std::string> ParallelReplicaJob::run(void)
{
    ...
    *trajectory = *reactant;
    ...
    // Hyperdynamics object initialization
    Hyperdynamics* boostPot = nullptr;
    printf("The biasPotential is: %s\n",parameters->biasPotential.c_str());
    if (parameters->biasPotential == "bond_boost"){
        boostPot = new BondBoost(trajectory, parameters);
        boostPot->initialize();
    ...
    int stateCheckInterval =
        int(floor(parameters->parrepStateCheckInterval / parameters->mdTimeStep + 0.5));
    int recordInterval =
        int(floor(parameters->parrepRecordInterval / parameters->mdTimeStep + 0.5));
    ...
    for (int step = 1; step <= parameters->mdSteps; step++) {
        ...
        double kinT =
            (2.0 * kinE / (3*trajectory->numberOfFreeAtoms()) / parameters->kB);
        ...
        // check for a transition every stateCheckInterval
        ...
        if (step % stateCheckInterval == 0 || step == parameters->mdSteps) {
            ...
            // same number of force calls will be performed on average
                ...
                if (step+1 == parameters->mdSteps && transitionTime == 0) {
    ...
    int decorrelationSteps =
        int(floor(parameters->parrepCorrTime / parameters->mdTimeStep + 0.5));
